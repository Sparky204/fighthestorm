...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['Scripted By Sparky','Fight The Storm','FTS System','SUB TO KAKASHI','/help'];
    var i = -1;
    var j = 0;
...
client.on('message', function(msg) {
 ...
 client.on('message', message => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .setColor('RANDOM')
            .setTitle('``INFO Sparky Community ©`` ')
 ...
 client.on('message', message => {
      ...
      message.reply(`All Members count =**${message.guild.memberCount}**`);
    });
    client.on('message', message => {
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'Sparky Community ©\'s translator',
...
message.channel.send(
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/uxkfDYY**")
...
client.on('message', message => {
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply(' **__You Dont Have Permisson__**');
              message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: false
...
client.on('guildCreate', guild => {
  ...
  .setDescription(`**
  New Server Add Sparky Community © ?
...
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=435392018693488641&scope=bot&permissions=2146958591')
  .setDescription(`**
  Server Kicked Sparky Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
...
client.on('message', message => {
  ...
  if (message.author.id == 571051618036875294) return;


if (message.content.startsWith(prefix + 'playing')) {
if (message.author.id !== '571051618036875294') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
...
if (message.content.startsWith(prefix + "rank")) {
    ...
    .addField("EXP :",Math.floor(EpicEdiTeD[message.author.id].Xp))
